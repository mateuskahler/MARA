/**
	 ...
	 * 
	 * Definição de uma fonte de tensão controlada por corrente
	 * 
	Mateus Kahler
*/

#ifndef SIMULADOR_ESTOQUE_FONTEVCONTROLADAI_INCLUSA
#define SIMULADOR_ESTOQUE_FONTEVCONTROLADAI_INCLUSA sim
...
struct FonteVControladaI : public Circuito::Componente
{
	NUM_T alfa; // ganho
	FonteVControladaI(std::string, NUMs_t parametros)
	{
		alfa = parametros[0];
	};
	...
	Circuito::Modelo_Ramificado modelo_inicial(Condicionamento::Contexto_Analise const &)override
	{
		...
		M.circuito.Adiciona(Circuito::Ramo_Base::Cria_DDP_Controlada_Corrente(alfa), 0, 1);
		return M;
	}
	...
	{
		return std::string("Fonte de DDP controlada por corrente");
	}
};

} } //fim namespaces Circuito::Componentes

#endif //fim header guard SIMULADOR_ESTOQUE_FONTEVCONTROLADAI_INCLUSA