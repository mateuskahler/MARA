/**
	 ...
	 * 
	 * Definição de uma fonte de corrente controlada por diferença de potencial
	 * 
	Mateus Kahler
*/

#ifndef SIMULADOR_ESTOQUE_FONTEICONTROLADAV_INCLUSA
#define SIMULADOR_ESTOQUE_FONTEICONTROLADAV_INCLUSA sim
...
struct FonteIControladaV : public Circuito::Componente
{
	NUM_T alfa; // ganho
	FonteIControladaV(std::string, NUMs_t parametros)
	{
		alfa = parametros[0];
	};
	...
	Circuito::Modelo_Ramificado modelo_inicial(Condicionamento::Contexto_Analise const &)override
	{
		...
		M.circuito.Adiciona(Circuito::Ramo_Base::Cria_Corrente_Controlada_DDP(alfa), 0, 1);
		return M;
	}
	...
	{
		return std::string("Fonte de corrente controlada por potencial");
	}
};

} } //fim namespaces Circuito::Componentes

#endif //fim header guard SIMULADOR_ESTOQUE_FONTEICONTROLADAV_INCLUSA